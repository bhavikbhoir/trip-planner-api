...
exports.handler = async (event) => {
  ...
  const tipId = event.pathParameters?.tipId
  if (!tripId || !tipId) return err(400, 'tripId and tipId are required')

  const member = await db.get(`TRIP#${tripId}`, `MEMBER#${userId}`)
  if (!member) return err(403, 'Not a member of this trip')

  await db.del(`TRIP#${tripId}`, `TIP#${tipId}`)

  return ok(200, { deleted: true, tipId })
}
