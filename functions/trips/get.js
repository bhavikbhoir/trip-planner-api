...
exports.handler = async (event) => {
  ...
  const { trip, members, logistics, bookings, plans, suggestions, approvals, eventCompletions, picks, expenses, tips } = await getTripAggregate(tripId)
  if (!trip) return err(404, 'Trip not found')

  const isMember = members.some((m) => m.userId === userId)
  if (!isMember) return err(403, 'Not a member of this trip')

  return ok(200, { trip, members, logistics, bookings, plans, suggestions, approvals, eventCompletions, picks, expenses, tips })
}
