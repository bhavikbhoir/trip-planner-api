              ...
              items: {
                ...
                properties: {
                  ...
                  alternatives: {
                    type: 'array',
                    description:
                      'Optional — ONLY for restaurant/meal events (icon "food"). Provide 1-2 alternative venues the group could pick instead, so they can swap without regenerating. Each should be a genuinely different real option (different price tier or cuisine), not a near-duplicate. Omit entirely for non-meal events.',
                    items: {
                      type: 'object',
                      properties: {
                        title: { type: 'string', description: 'Venue name' },
                        cuisine: { type: 'string', description: 'Short cuisine/style tag, e.g. "Vegan · Mediterranean"' },
                        note: { type: 'string', description: 'One-line reason to consider it — price, vibe, tradeoff' },
                        costPerPerson: { type: 'number', description: 'Whole-number USD estimate' },
                        lat: { type: 'number', description: 'Optional map pin' },
                        lng: { type: 'number', description: 'Optional map pin' },
                      },
                      required: ['title'],
                    },
                  },
