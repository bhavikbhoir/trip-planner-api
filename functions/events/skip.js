...
// Done/Skipped/Swapped are mutually exclusive per event — an event is at
// most one of them at a time, enforced here via a single atomic
// transactWrite rather than a put-then-two-deletes (which could leave an
// event transiently double-counted, or counted twice, if a request failed
// partway through).
exports.handler = async (event) => {
  ...
  const item = { pk, sk: `SKIPPED#${eventId}`, tripId, eventId, skippedBy: userId, skippedAt: new Date().toISOString(), note }
  ...
  return ok(200, { status: 'skipped', eventId, note: note || null })
}
